...
//	private int newVertexID;
	...
	public TrimeshMaker(int numVertices_, int numTris_, int numCreases_)
	{
		...
		this.textureBuffer = Buffers.newDirectFloatBuffer(2 * this.numVertices);
...
//	}
	...
	public void addTriangle(Point3f v0, Point3f v1, Point3f v2, Point2f t0, Point2f t1, Point2f t2, int i0_, int i1_, int i2_)
	{
...
//		int i2 = this.remapVertexID(i2_);
		...
		//add t0:
		this.textureBuffer.put(2*i0 + 0, t0.x);
		this.textureBuffer.put(2*i0 + 1, t0.y);
		...
		//add t1:
		this.textureBuffer.put(2*i1 + 0, t1.x);
		this.textureBuffer.put(2*i1 + 1, t1.y);
		...
		//add t2:
		this.textureBuffer.put(2*i2 + 0, t2.x);
		this.textureBuffer.put(2*i2 + 1, t2.y);
	...
	public Trimesh getMesh()
	{
		...
		this.mMesh.setTexCoordData(this.textureBuffer);
